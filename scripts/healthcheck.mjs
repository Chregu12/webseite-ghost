...
async function main() {
  ...
  await check("Frontend: builder dashboard list API is protected (401)", async () => {
    const res = await fetch(`${SITE_URL}/api/builder/list`);
    if (res.status !== 401) throw new Error(`expected 401, got ${res.status}`);
  });
  ...
  await check("Frontend: builder snippets API is protected (401)", async () => {
    const res = await fetch(`${SITE_URL}/api/builder/snippets`);
    if (res.status !== 401) throw new Error(`expected 401, got ${res.status}`);
  });

  await check("Frontend: builder site-config API is protected (401)", async () => {
    const res = await fetch(`${SITE_URL}/api/builder/siteconfig?lang=de`);
    if (res.status !== 401) throw new Error(`expected 401, got ${res.status}`);
  });
